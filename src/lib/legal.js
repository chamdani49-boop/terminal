...
export async function getConsentVersion(env, userId) {
  ...
  try {
    ...
    const row = await env.DB.prepare('SELECT version FROM consents WHERE user_id = ?').bind(userId).first();
    return (row && row.version) || 0;
  } catch (_) { return 0; }
}

/** Apakah user sudah menyetujui versi terbaru. FAIL-OPEN saat error. */
export async function hasAcceptedCurrent(env, userId) {
  if (!env.DB || !userId) return true;   // tanpa DB → jangan ganggu
  try {
    const v = await getConsentVersion(env, userId);

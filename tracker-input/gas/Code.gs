...
/**
 * doGet: dua mode
 *   1) Tanpa parameter          → ping "service hidup" (backward-compatible).
 *   2) ?action=list&token=XXX  → return baris `status=approved` sbg JSON.
 *                                Dipakai `scripts/build-tracker.js` untuk
 *                                menghasilkan `public/tracker.json`.
 *
 * Parameter query yang didukung untuk action=list:
 *   token   (wajib) — harus sama dgn TOKEN di atas.
 *   since   (opsional, unix ms) — hanya baris dgn timestamp > since.
 *   limit   (opsional, default 5000) — batas maksimum baris.
 */
function doGet(e) {
  try {
    var q = (e && e.parameter) || {};
    ...
    if (action === 'list') {
      if (!q.token || q.token !== TOKEN) {
        return _json({ ok: false, error: 'unauthorized' });
      }
      var since = q.since ? parseInt(q.since, 10) : 0;
      var limit = q.limit ? parseInt(q.limit, 10) : 5000;
      return _json(_listApproved(since, limit));
    }
...
/**
 * Baca sheet, filter `status=approved`, return objek array.
 * Format tiap item = mapping kolom HEADERS → nilai (nilai kosong = '').
 * Tanggal timestamp dikonversi ke ISO string; kolom 'tanggal' juga
 * dinormalisasi ke ISO date bila berupa Date.
 */
function _listApproved(since, limit) {
  var sh = _sheet();
  var last = sh.getLastRow();
  if (last <= 1) {
    return { ok: true, count: 0, headers: HEADERS.slice(), items: [] };
  }
  // Ambil semua data sekaligus (lebih cepat daripada baris-per-baris).
  ...
  for (var i = 0; i < range.length; i++) {
    var row = range[i];
    if (String(row[statusIdx] || '').toLowerCase() !== 'approved') continue;
    ...
    var obj = { _row: i + 2, _ts: ts };
    for (var c = 0; c < HEADERS.length; c++) {
      var key = HEADERS[c];
      var val = row[c];
      if (val instanceof Date) {
        val = val.toISOString();
      } else if (key === 'tanggal' && val && typeof val === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(val.trim())) {
        val = val.trim();
      }
